...
| `src/iface/socket_set.rs` | `get_mut` | 115 | 21 | OBLIGATION | call to core[a8c6]::panicking::panic_fmt may panic: MightPanic(NoMIRAv |
| `src/wire/tcp.rs` | `add` | 43 | 7 | OBLIGATION | call to core[a8c6]::panicking::panic_fmt may panic: MightPanic(NoMIRAv |
| `src/iface/packet.rs` | `emit_payload` | 74 | 6 | OBLIGATION | call to core[a8c6]::result::{impl#0}::unwrap may panic: MightPanic(Tra |
...
| `src/iface/interface/sixlowpan.rs` | `ipv6_to_sixlowpan` | 425 | 5 | OBLIGATION | refinement type error |
...
| `src/wire/sixlowpan/mod.rs` | `resolve` | 66 | 5 | OBLIGATION | refinement type error |
...
| `src/wire/ipv6.rs` | `set_flow_label` | 529 | 4 | OBLIGATION | assertion might fail: possible out-of-bounds access |
...
| `src/wire/tcp.rs` | `parse` | 658 | 3 | OBLIGATION | call to core[a8c6]::iter::traits::iterator::Iterator::for_each may pan |
...
| `src/iface/interface/ipv6.rs` | `get_source_address_ipv6` | 31 | 2 | OBLIGATION | call to core[a8c6]::panicking::panic may panic: MightPanic(NoMIRAvaila |
| `src/iface/interface/sixlowpan.rs` | `dispatch_sixlowpan` | 295 | 2 | OBLIGATION | refinement type error |
...
| `src/iface/socket_set.rs` | `add` | 63 | 2 | OBLIGATION | call to core[a8c6]::iter::adapters::enumerate::{impl#1}::next may pani |
...
| `src/storage/ring_buffer.rs` | `get_allocated` | 359 | 2 | OBLIGATION | call to managed[ba83]::slice::{impl#2}::deref may panic: MightPanic(No |
...
| `src/wire/icmpv6.rs` | `emit` | 777 | 2 | OBLIGATION | call to xarxa_driver[1613]::{impl#1}::tx may panic: MightPanic(NoMIRAv |
...
| `src/wire/ipv6.rs` | `mask` | 163 | 2 | OBLIGATION | refinement type error |
...
| `src/wire/ipv6hbh.rs` | `emit` | 100 | 2 | OBLIGATION | call to heapless[ea1e]::vec::{impl#11}::into_iter may panic: MightPani |
...
| `src/wire/sixlowpan/iphc.rs` | `flow_label_field` | 204 | 2 | OBLIGATION | call to core[a8c6]::panicking::panic may panic: MightPanic(NoMIRAvaila |
...
| `src/wire/sixlowpan/nhc.rs` | `dst_port` | 550 | 2 | OBLIGATION | call to core[a8c6]::panicking::panic may panic: MightPanic(NoMIRAvaila |
...
| `src/iface/interface/ipv6.rs` | `process_hopbyhop` | 265 | 1 | OBLIGATION | call to core[a8c6]::net::ip_addr::{impl#20}::is_multicast may panic: M |
...
| `src/storage/packet_buffer.rs` | `dequeue` | 210 | 1 | OBLIGATION | call to core[a8c6]::option::{impl#0}::unwrap may panic: MightPanic(Tra |
| `src/storage/packet_buffer.rs` | `peek` | 225 | 1 | OBLIGATION | call to core[a8c6]::option::{impl#0}::unwrap may panic: MightPanic(Tra |
| `src/storage/ring_buffer.rs` | `get_unallocated` | 302 | 1 | OBLIGATION | refinement type error |
...
| `src/wire/dhcpv4.rs` | `emit` | 843 | 1 | OBLIGATION | call to core[a8c6]::slice::iter::{impl#166}::next may panic: MightPani |
...
| `src/wire/icmpv6.rs` | `parse` | 661 | 1 | OBLIGATION | call to xarxa_driver[1613]::{impl#1}::rx may panic: MightPanic(NoMIRAv |
...
| `src/wire/ieee802154.rs` | `src_pan_id` | 542 | 1 | OBLIGATION | refinement type error |
...
| `src/wire/ipv6.rs` | `src_addr` | 477 | 1 | OBLIGATION | call to core[a8c6]::result::{impl#0}::unwrap may panic: MightPanic(Tra |
...
| `src/wire/mld.rs` | `set_qrv` | 138 | 1 | OBLIGATION | assertion might fail: possible out-of-bounds access |
...
| `src/wire/ndisc.rs` | `target_addr` | 82 | 1 | OBLIGATION | refinement type error |
| `src/wire/ndisc.rs` | `dest_addr` | 109 | 1 | OBLIGATION | call to core[a8c6]::result::{impl#0}::unwrap may panic: MightPanic(Tra |
...
| `src/wire/ndiscoption.rs` | `parse` | 449 | 1 | OBLIGATION | refinement type error |
...
| `src/wire/sixlowpan/iphc.rs` | `buffer_len` | 763 | 1 | OBLIGATION | call to core[a8c6]::array::{impl#15}::index may panic: MightPanic(Tran |
| `src/wire/sixlowpan/nhc.rs` | `src_port` | 521 | 1 | OBLIGATION | assertion might fail: possible out-of-bounds access |
...
| `src/wire/tcp.rs` | `emit` | 1014 | 1 | OBLIGATION | call to xarxa_driver[1613]::{impl#1}::tx may panic: MightPanic(NoMIRAv |
